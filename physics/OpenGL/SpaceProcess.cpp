...
void SpaceProcess::overrideGLShapeDrawer(GL_ShapeDrawer* shapeDrawer) {
...
void SpaceProcess::setCameraDistance(float dist) {
    camDist = dist;
}

float SpaceProcess::getCameraDistance() {
    return camDist;
}

void SpaceProcess::toggleIdle() {
    if (m_idle) {
        m_idle = false;
    } else {
        m_idle = true;
...
void SpaceProcess::updateCamera(float dt) {

    //interpolate cameraPosition cameraPositionNext
    camPos = (1.0 - cameraSpeed) * camPos + (cameraSpeed) * m_cameraPositionNext;
    camTarget = (1.0 - cameraSpeed) * camTarget + (cameraSpeed) * camTargetNext;
    ...
    camPos[0] = eyePos.getX();
    camPos[1] = eyePos.getY();
    camPos[2] = eyePos.getZ();
    camPos += camTarget;
...
void SpaceProcess::stepLeft() {
    //m_azi -= STEPSIZE; if (m_azi < 0) m_azi += 360;
    camTargetNext += m_cameraRight * 0.1;
    updateCamera(0);
}

void SpaceProcess::stepRight() {
    //m_azi += STEPSIZE; if (m_azi >= 360) m_azi -= 360;
    camTargetNext -= m_cameraRight * 0.1;
    updateCamera(0);
}
...
void SpaceProcess::zoomOut() {
    camDist += 0.4;
    updateCamera(0);

}

void SpaceProcess::reshape(int w, int h) {
...
void SpaceProcess::onKeyboard(unsigned char key, int x, int y) {
    ...
    switch (key) {
        ...
        case 'i': toggleIdle();
            break;
        case 'g': m_enableshadows = !m_enableshadows;
            break;
        case 'u': m_shapeDrawer->enableTexture(!m_shapeDrawer->enableTexture(false));
            break;
        case 'h':
...
void SpaceProcess::setDebugMode(int mode) {
    m_debugMode = mode;
    if (getDynamicsWorld() && getDynamicsWorld()->getDebugDrawer())
...
void SpaceProcess::setShootBoxShape() {
    if (!m_shootBoxShape) {
        m_shootBoxShape = new btBoxShape(btVector3(.2f, .2f, .2f));
    }
}

void SpaceProcess::shootBox(const btVector3& destination) {
...
btVector3 SpaceProcess::getRayTo(int x, int y) {
    ...
    if (m_ortho) {
        ...
        btVector3 p(0, 0, 0);
        p.setValue((1.0f - u) * lower.getX() + u * upper.getX(), (1.0f - v) * lower.getY() + v * upper.getY(), camTarget.getZ());
        return p;
    }
                    ...
                    if (rayCallback.hasHit()) {
                        ...
                        if (body) {
                            ...
                            if (!(body->isStaticObject() || body->isKinematicObject())) {
                                ...
                                btPoint2PointConstraint* p2p = new btPoint2PointConstraint(*body, touchPosLocal);
                                ...
                                gHitPos = touchPosWorld;

                                gOldPickingDist = (touchPosWorld - rayFrom).length();
...
btRigidBody* SpaceProcess::localCreateRigidBody(float mass, const btTransform& startTransform, btCollisionShape* shape) {
...
void SpaceProcess::setOrthographicProjection() {
...
void SpaceProcess::resetPerspectiveProjection() {
...
void SpaceProcess::displayProfileString(int xOffset, int yStart, char* message) {
    glRasterPos3f(xOffset, yStart, 0);
    GLDebugDrawString(xOffset, yStart, message);
}

void SpaceProcess::showProfileInfo(int& xOffset, int& yStart, int yIncr) {
#ifndef BT_NO_PROFILE
...
void SpaceProcess::renderscene(int pass) {
    btScalar m[16];
    btMatrix3x3 rot;
    rot.setIdentity();
    ...
    for (int i = 0; i < numObjects; i++) {
        ...
        switch (pass) {
            case 0: m_shapeDrawer->drawOpenGL(m, colObj->getCollisionShape(), NULL, wireColor, getDebugMode(), aabbMin, aabbMax);
                break;
            case 1: m_shapeDrawer->drawShadow(m, m_sundirection*rot, colObj->getCollisionShape(), aabbMin, aabbMax);
                break;
            case 2: m_shapeDrawer->drawOpenGL(m, colObj->getCollisionShape(), NULL, wireColor * 0.3, 0, aabbMin, aabbMax);
                break;
        }
    }
}
...
void SpaceProcess::clientResetScene() {
#ifdef SHOW_NUM_DEEP_PENETRATIONS
    ...
    if (dynamicsWorld) {
        ...
        dynamicsWorld->getBroadphase()->resetPool(getDynamicsWorld()->getDispatcher());
